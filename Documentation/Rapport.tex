...
\begin{document}
	...
	\chapter{Cahier des charges}
		Ce chapitre détaille toute la phase de conception du projet. Elle reprend le Game Design Document que nous avons rédigé lors de l'étude préalable.
		...
		\section{Structure du jeu}
			...
			\subsection{Moteur de carte}
				La gestion de la carte se fera uniquement en C++. Il existera pour cela différentes classes gérant la carte, les tuiles et le tileset.
					...
					\begin{table}[H]
						\begin{small}
							\begin{tabular}{| c | c | c |p{5cm}|}
								...
								\hline
								Terre Fertile & - & Oui & Terre pouvant etre cultivée pour devenir un \textbf{Champ}.\\
								\hline
		...
		\section{\'Eléments graphiques}
			...
			\subsection{Les tuiles}
				\label{Tuile}
				Les tuiles seront des images de 32x32 ou 16x16 pixels au format PNG et seront créées par l'équipe.\\
				\begin{itemize}[label=$\bullet$]
			...
			\subsection{Les entités}
				Chaque entité aura plusieurs positions possibles. Pour chacun d'elle, un ou plusieurs éléments graphiques seront créés par l'équipe.\\
			...
			\subsection{Interface utilisateur}
				...
				\begin{figure}[H]
					\begin{center}
						\includegraphics[scale=0.35]{img/interfaceSample.png}
					\end{center}
					\label{fig:interface}
					\caption{Présentation de l'interface du jeu}
				\end{figure}
		...
		\section{Gestion des fichiers}
			...
			\subsection{Format des Fichiers}
				...
				\subsubsection{Tileset}
					Le tileset sera une unique image au format PNG, créé par l'équipe, représentant l'ensemble des tuiles possibles, accompagnée d'un fichier de configuration au format INI. Chaque type de sol constituera une ligne du tileset, avec toutes les variantes dépendant des rebords entre tuiles (ce fichier sera présenté plus en détails dans la section \ref{TilesetDev}, page \pageref{TilesetDev}).
	...
	\chapter{Développement}
		...
		\section{Moteur de carte}
			...
			\subsection{Le Tileset}
				...
				\begin{itemize}[label=$\bullet$]
					\item \textbf{path} : indique le chemin du fichier de texture du tileset\\
					\item \textbf{tilesize} : est suivi des dimensions (en pixel, largeur puis hauteur) d'une tuile\\
					\item \textbf{groundColumnCount} : nombre de colonnes pour chaque sol dans le fichier de texture (toutes les associations de rebords)\\
					\item \textbf{elementColumnCount} : nombre de colonnes pour chaque élément dans le fichier de texture (tous les types de sols)\\
					\item \textbf{ground} : définit un sol et tous ses attributs\\
					\item \textbf{element} : définit un élément et tous ses attributs (height est la hauteur en nombre de cases dans la texture)\\
					\item \textbf{ashes} : fourni une texture pour les cendres (lorsque des éléments brûlent) (cf GDD)
				\end{itemize}
				...
				\begin{figure}[H]
					\begin{center}
						\includegraphics[scale=0.5]{img/groundBorders.png} 
					\end{center}
					\label{fig:border}
					\caption{Présentation du codage des bordures des sols}
				\end{figure}
