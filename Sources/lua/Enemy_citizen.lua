	...
	if self:isAttacked() then
		...
		while (i<=taille) do -- priorit̩ aux citoyens risque de tout parcourir(sinon on fait un || entre animaux agressifs  et citoyens.
			...
			elseif (not((entites:isAttackMe(entities:getID(indice))) and (entites:getAngle(indice)==self:getAngle()))) then
				if (entites:isAttackMe(entities:getID(i))) and (entites:getType(i)=="HUNTER") then
					indice=i

				elseif (not(entites:isAttackMe(entities:getID(indice)) and entites:getType(indice)=="HUNTER")) then
					if (entites:isAttackMe(entities:getID(i))) and (entites:getType(i)=="ALLY_CITIZEN") then
						indice=i

					elseif (not(entites:isAttackMe(entities:getID(indice)) and entites:getType(indice)=="ALLY_CITIZEN")) then
