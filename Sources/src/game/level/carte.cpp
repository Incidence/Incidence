...
Sol* Carte::getSol(sf::Vector2i position) const {
	...
	if(id < m_sols.size() || id > m_sols.size()) {
		return m_sols[id];
	}
	else {
		return NULL;
...
Element* Carte::getElement(sf::Vector2i position) const {
	...
	int id = getId(position);
	if(id < m_elements.size() || id > m_sols.size()) {
...
void Carte::generate() {
			...
			if(i != 0 || j != 0) {
				...
				for(int k(0) ; k < m_tileset.getNombreSol() ; ++k) {
					...
					if(l_sol_tmp != NULL) {
						bool add = true;
						if(i > 0) {
							if(!(m_sols[i - 1 + j * width]->getType() == k 
								|| m_sols[i - 1 + j * width]->isBord(k) 
								|| l_sol_tmp->isBord(m_sols[i - 1 + j * width]->getType()))) {
								add = false;
							}
						}
						if(j > 0) {
							if(!(m_sols[i + (j - 1) * width]->getType() == k
								|| m_sols[i + (j - 1) * width]->isBord(k) 
								|| l_sol_tmp->isBord(m_sols[i + (j - 1) * width]->getType()))) {
								add = false;
							}
						}
						if(add) {
							if(i > 0 && m_sols[i - 1 + j * width]->getType() == k && (m_sols[i - 1 + j * width]->getNom()).compare("eau") != 0) {
								l_bords.push_back(k);
							}
							if(j > 0 && m_sols[i + (j - 1) * width]->getType() == k && (m_sols[i + (j - 1) * width]->getNom()).compare("eau") != 0) {
								l_bords.push_back(k);
							}
							l_bords.push_back(k);
						}
	...
	for(int j(1) ; j < height-1 ; ++j) {
		for(int i(1) ; i < width-1 ; ++i) {
			...
			//===== SOL : passe 2 (homogénéisation des zones) =====
			
			Sol* current = m_sols[i + j * width];
			Sol* round[4] = { m_sols[i - 1 + j * width], m_sols[i + (j + 1) * width], m_sols[i + (j - 1) * width], m_sols[i + 1 + j * width] };
			
			if( ( current->isBord(round[0]->getType()) 
				  && current->isBord(round[1]->getType()) 
				  && current->isBord(round[2]->getType()) 
				  && current->isBord(round[3]->getType()) 
				) || (
				  round[0]->isBord(current->getType()) 
				  && round[1]->isBord(current->getType()) 
				  && round[2]->isBord(current->getType()) 
				  && round[3]->isBord(current->getType()) 
				) ) {
				
				int k(0), random(rand());
				while(k < 4) {
					
					int SOL_type = round[(k+random)%4]->getType();
					bool SOL_bordures[4] = {false,false,false,false};
					...
					if(l_sol != NULL) {
						...
						bool change = true;
						for(int l(0) ; l < 4 ; ++l) {
							if(!(round[l]->getType() == SOL_type || l_sol->isBord(round[l]->getType()) || round[l]->isBord(SOL_type))) {
								change = false;
							}
						}
						if(change) {
	...
	for(int j(0) ; j < height ; ++j) {
		for(int i(0) ; i < width ; ++i) {
			...
			//===== SOL : passe 3 (bordures) =====
			
			int SOL_type = m_sols[i + j * width]->getType();
			bool SOL_bordures[4] = {false,false,false,false};
			
			SOL_bordures[0] = (i > 0 && m_sols[i + j * width]->isBord(m_sols[i - 1 + j * width]->getType()));
			SOL_bordures[1] = (j < height-1 && m_sols[i + j * width]->isBord(m_sols[i + (j + 1) * width]->getType()));
			SOL_bordures[2] = (j > 0 && m_sols[i + j * width]->isBord(m_sols[i + (j - 1) * width]->getType()));
			SOL_bordures[3] = (i < width-1 && m_sols[i + j * width]->isBord(m_sols[i + 1 + j * width]->getType()));
